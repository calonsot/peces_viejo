...
<div class="view">
	<?php foreach ($peces as $k => $pez) {
		...
		{
			if($j->Nivel1==1)
				array_push($estados_conservacion, ($j->nombre).CHtml::link(' (IUCN)', "http://www.iucnredlist.org/about/introduction", array("style"=>"color:#584B05;font-size:10px;", "target" => "_blank")));
				
			if($j->Nivel1==2)
				array_push($estados_conservacion, ($j->nombre).CHtml::link(' (CITES)', "http://www.cites.org/esp/app/index.phpn", array("style"=>"color:#584B05;font-size:10px;", "target" => "_blank")));
