...
<div class="view">
	<?php foreach ($peces as $k => $pez) {
		...
		{
			if($j->Nivel1==1)
				array_push($estados_conservacion, utf8_decode($j->nombre).CHtml::link(' (IUCN)', "http://www.biodiversidad.gob.mx/especies/catRiesMundo.html", array("style"=>"color:#584B05;font-size:10px;", "target" => "_blank")));
				
			if($j->Nivel1==2)
				array_push($estados_conservacion, utf8_decode($j->nombre).CHtml::link(' (CITES)', "http://www.biodiversidad.gob.mx/planeta/cites/index.html", array("style"=>"color:#584B05;font-size:10px;", "target" => "_blank")));
